...
const SelectedFeed = () => {
    ...
    return (
        <main className={styles.card}>
            ...
            <span className={`${styles.status} text text_type_main-default mt-3`}>{order?.status === 'done' ? `Выполнен` : `Готовится`}</span>
            <span className={`${styles.subtitle} text text_type_main-medium  mt-15`}>Cостав:</span>
            <ul className={styles.scroll} >
                {order && ingredients && includesIngredients(ingredients, order?.ingredients).map(element => {
